...
ViewFinder.drag=function(e){if(ViewFinder.dragging){ViewFinder.preventHighlight(e);var left=ViewFinder.dragging===Dashboard.Elements.viewFinderGrabLeft,other=left?Dashboard.Elements.viewFinderGrabRight:Dashboard.Elements.viewFinderGrabLeft,tsDrag=ViewFinder.getTimestampFromOffset(e.pageX),tsOther=ViewFinder.getTimestamp(other),domain=Dashboard.Data.filterDomain(tsDrag,tsOther);count=domain.reduce(function(last,current){return Math.min(last.data.length,current.data.length);});if(count>2&&(left?(tsOther-tsDrag):(tsDrag-tsOther))>=0){var width=Dashboard.Elements.viewFinder.clientWidth,offset=ViewFinder.getOffset(),percent=(e.pageX-offset.left)/width;ViewFinder.dragging.parentNode.style.width=(left?percent:(1-percent))*width+'px';ViewFinder.dragging.setAttribute('data-timestamp',tsDrag);ViewFinder.updateChart();}}}
